...
2.  **Follow the graph structure.** Do not create paths that do not exist. The correct way to find downtime for a machine is `(:Machine)-[:RECORDED_DOWNTIME_EVENT]->(:MachineDowntimeEvent)`.
...
5.  **Handle dates correctly.** For questions about a time period like 'last month', use a `WHERE` clause with `date()` and `duration()`. For maintenance, filter on the `actual_finish_date` property. Example: `WHERE wo.actual_finish_date >= date() - duration({months: 1})`.
